...
exports.BrowserPool = exports.SITE_CONFIGS = void 0;
exports.initializeBrowserPool = initializeBrowserPool;
exports.scrapeGeneric = scrapeGeneric;
exports.scrapeMFC = scrapeMFC;
const puppeteer_1 = __importDefault(require("puppeteer"));
// Enhanced fuzzy string matching for robust Cloudflare detection
function fuzzyMatchesPattern(text, pattern, threshold = 0.8) {
    if (!text || !pattern)
        return false;
...
function detectCloudflareChallenge(title, bodyText, patterns) {
...
class BrowserPool {
    ...
    static async initialize() {
        ...
        for (let i = 0; i < this.POOL_SIZE; i++) {
            try {
                const browser = await puppeteer_1.default.launch(this.getBrowserConfig());
    ...
    static async getBrowser() {
        ...
        if (!browser) {
            if (this.emergencyBrowserCount < this.MAX_EMERGENCY_BROWSERS) {
                ...
                try {
                    const emergencyBrowser = await puppeteer_1.default.launch(this.getBrowserConfig());
    ...
    static async replenishPool() {
        // Use a lock to prevent concurrent replenishment attempts
        if (this.replenishLock || this.browsers.length >= this.POOL_SIZE)
            return;
        try {
...
BrowserPool.POOL_SIZE = 3; // Keep 3 browsers ready
BrowserPool.MAX_EMERGENCY_BROWSERS = 5; // Limit emergency browser creation
BrowserPool.isInitialized = false;
BrowserPool.replenishLock = false; // Prevent concurrent replenishment
BrowserPool.emergencyBrowserCount = 0;
...
async function scrapeGeneric(url, config) {
    ...
    try {
        ...
        if (config.cloudflareDetection) {
            ...
            if (challengeDetected) {
                ...
                await page.waitForFunction((patterns) => {
                    ...
                    return !patterns.some(pattern => currentTitle.includes(pattern.toLowerCase()) ||
                        currentBodyText.includes(pattern.toLowerCase()));
                }, { timeout: 10000 }, challengePatterns // Matches test expectation
                ).catch(() => {
            ...
            try {
                ...
                if (selectors.manufacturerSelector) {
                    if (selectors.manufacturerSelector.includes(':contains(')) {
                        ...
                        const manufacturerElement = document.querySelector(selectors.manufacturerSelector);
                        if (manufacturerElement && manufacturerElement.textContent) {
                            data.manufacturer = manufacturerElement.textContent.trim();
                        }
                    }
                ...
                if (selectors.scaleSelector) {
                    const scaleElement = document.querySelector(selectors.scaleSelector);
        ...
        const isCriticalError = criticalErrors.some(errorType => error.message.includes(errorType) || error.message === errorType);
        if (isCriticalError) {
            throw error;
        }
...
async function scrapeMFC(url) {
    return scrapeGeneric(url, exports.SITE_CONFIGS.mfc);
}
// Graceful shutdown
